defmodule LiveAdmin.Components do
  ...
  def field_icons(assigns) do
    assigns = assign(assigns, :id, "modal-#{Map.fetch!(assigns.record, LiveAdmin.primary_key!(assigns.resource))}-#{assigns.field}")
    ...
    <div>
        ...
        >
          ...
          <div class="modal-body">
            <div class="detail-section-content">
              {@record |> Map.fetch!(@field) |> safe_render()}
            </div>
          </div>
        </div>
      </div>
      <div class="field-value-icons">
        <span
          class="expand-icon"
          phx-click={JS.show(to: "#" <> @id, display: "flex")}
        >
          <svg
            width="14"
            height="14"
